#!/usr/bin/env python3
"""Prepare/install the verified crate batch UI, or restore its exact pack backup.

Dry run is the default. --apply requires H1Z1 to be closed. Only MarketplaceWindow.gfx in
Assets_164.pack changes; the existing helmet patch in Assets_177.pack is untouched.
...
OLD = 'dc8e03de521fdf9a7e5f5f2f24716d897fd1f2a50c24b9cf209895ec1065252c'
PREVIOUS = '30c9081d9fe1eb738572e7688f82491ead0ff64851a73f1ddd608001569e3b9a'
NEW = '828a2a9cd4eff0012bae57f7c6b6408bf26deb1228d97dcc7ae2918ddf861989'
NAME = 'MarketplaceWindow.gfx'
PACK = 'Assets_164.pack'
...
def prepare_update(original, patch, old_hash=(OLD, PREVIOUS), new_hash=NEW):
    ...
    if len(targets) != 1:
        raise ValueError('Expected exactly one MarketplaceWindow.gfx')
    _, index, offset, size, crc = targets[0]
    source = original[offset:offset + size]
    if zlib.crc32(source) != crc:
        raise ValueError('Existing MarketplaceWindow CRC does not match the index')
    if digest(source) == new_hash:
        return original, index, len(before) - 1
    accepted = (old_hash,) if isinstance(old_hash, str) else tuple(old_hash)
    if digest(source) not in accepted:
        raise ValueError('MarketplaceWindow has another modification; refusing to replace it')
    ...
    for left, right in zip(before, entries(updated), strict=True):
        if left[0] == NAME:
            if bytes(updated[right[2]:right[2] + right[3]]) != patch:
                raise ValueError('Updated MarketplaceWindow verification failed')
        elif left != right:
...
def replace_verified(pack, replacement, expected_current_hash):
    descriptor, name = tempfile.mkstemp(prefix=pack.name + '.crate-unlock-', suffix='.tmp', dir=pack.parent)
    staged = Path(name)
    try:
...
def restore(manifest_path, apply):
    ...
    if current_hash == manifest['original_pack_sha256']:
        print('Original crate batch UI pack is already restored')
        return
...
def main():
    ...
    if pack.name != PACK:
        raise ValueError('Expected the August Assets_164.pack')
    original = pack.read_bytes()
    updated, index, other_count = prepare_update(original, args.asset.read_bytes())
    if original == updated:
        print('Verified crate batch UI is already installed')
        return
    manifest = {
        'pack': str(pack), 'index_offset': index,
        'original_pack_sha256': digest(original), 'installed_pack_sha256': digest(updated),
        'original_asset_sha256': next(digest(original[offset:offset + size])
            for name, _, offset, size, _ in entries(original) if name == NAME),
        'installed_asset_sha256': NEW,
