...
spec = importlib.util.spec_from_file_location('bounty_installer', Path(__file__).resolve().parents[1] / 'install-bounty-lobby.py')
...
class BountyInstallerTests(unittest.TestCase):
    ...
    def test_restore_is_dry_run_by_default_and_refuses_later_changes(self):
        ...
        with tempfile.TemporaryDirectory(prefix='cranberry-bounty-installer-') as directory:
            root = Path(directory)
