...
{
    public static async Task<bool> ApplyAfterStartup(Process game, string directory, Action<string> log)
    {
        if (!OperatingSystem.IsWindows()) return false;
        try
        {
            if (!Environment.Is64BitProcess) throw new InvalidOperationException("The door helper requires a 64-bit launcher.");
            _ = game.SafeHandle;
            ...
            while (elapsed.Elapsed < TimeSpan.FromMinutes(30))
            {
                ...
                try
                {
                    ...
                    if (module is not null && string.Equals(Path.GetFullPath(module.FileName), expected, StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        {
                            // The main menu has no gameplay reticle. Only the initialized run state
                            // and verified instruction bytes are needed for this entity-independent hook.
                            _ = Inspect(Read(handle, image + FunctionRva, FunctionSize), image, (a, n) => Read(handle, a, n));
                            ready = image;
                        }
                    }
                }
                ...
                else if (elapsed.Elapsed - stableSince >= TimeSpan.FromSeconds(3))
                {
                    ...
                    {
                        log("Door update initialized and verified.");
                        return true;
                    }
                }
                await Task.Delay(1000).ConfigureAwait(false);
            }
            log("Door helper timed out waiting for an initialized client.");
        }
        ...
        {
            log("Door update could not initialize: " + ex.Message);
        }
        return false;
    }
    ...
    {
        using var mutex = new Mutex(false, @"Local\Cranberry.BidirectionalDoors." + game.Id);
        bool owned;
        ...
        try
        {
            ...
            try
            {
                ...
                if (!Read(write, (long)stub, code.Length).AsSpan().SequenceEqual(code)) throw new IOException("Door filter readback failed.");
                if (!VirtualProtectEx(write, stub, 4096, 0x20, out _) || !FlushInstructionCache(write, stub, (nuint)code.Length)) throw NativeError();

                // A five-byte detour must not be written while a client thread can execute it.
                ...
                if (status < 0) throw new IOException($"Could not pause door update ({status:X8}).");
                try
                {
                    ...
                    if (!VirtualProtectEx(write, (nint)entryAddress, EntrySize, 0x40, out uint protection)) throw NativeError();
                    ...
                    try
                    {
                        try
                        {
                            ...
                            if (Inspect(Read(write, image + FunctionRva, FunctionSize), image, (a, n) => Read(write, a, n)) != (long)stub)
                                throw new IOException("Door detour verification failed.");
                        }
                        catch
                        {
                            ...
                            if (Inspect(Read(write, image + FunctionRva, FunctionSize), image, (a, n) => Read(write, a, n)) != 0)
                                throw new IOException("Door detour rollback failed verification.");
                            retainStub = false;
                            throw;
                        }
                    }
                    ...
                    {
                        bool restored = VirtualProtectEx(write, (nint)entryAddress, EntrySize, protection, out _)
                            || VirtualProtectEx(write, (nint)entryAddress, EntrySize, protection, out _);
                        bool flushed = FlushInstructionCache(write, (nint)entryAddress, EntrySize);
                        if (!restored || !flushed) throw new IOException("Door page protection/cache cleanup failed.");
                    }
                }
                finally
                {
                    int resumed = NtResumeProcess(write);
                    if (resumed < 0 && NtResumeProcess(write) < 0) throw new IOException($"Could not resume game after door update ({resumed:X8}).");
                }
                return true;
            }
    ...
    private static bool ThreadInEntry(Process game, long entry)
    {
        ...
        foreach (ProcessThread thread in game.Threads)
        {
            ...
            try
            {
                ...
                if (rip >= entry && rip < entry + EntrySize) return true;
            }
    ...
    private static nint AllocateNear(SafeProcessHandle handle, long function)
    {
        ...
        throw new IOException("No nearby memory is available for the door filter.");
    }
    ...
    {
        if (address < 0x10000 || address >= (1L << 47) - size || size is < 1 or > FunctionSize) throw new InvalidDataException("Invalid bounded door read.");
